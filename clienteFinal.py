...
running = False
quit = reset = up = right = bottom = left = False
pos = ""
partida = ""
sendpos = False

def trata_mensagem(data):
    msgsrv = data.decode()
    dados = msgsrv.split(':')
    cod = int(dados[0])
    ...
    match cod:
        ...
        case 2:
            ...
            cells, fillcount, p1_score, p2_score, players, turn, player, next_turn, gameover = init(initt)
...
playerinit =  int(input('player(1, 2): '))
msg = "1:João"
...
# Inicia a função em uma thread separada
thread = threading.Thread(target=atualize)
thread.daemon = True  # Permite que o programa termine mesmo com a thread rodando
thread.start()
...
def reset_game():
    ...
    players = ['X', 'O']
    turn = 0  # Índice do jogador atual
    if playerinit == 2:
        turn = 1  # Inicializa as variáveis do jogo
...
def init(initt):
    if initt:
        ...
        initt = False
        gameover = False  # Indica se o jogo acabou
    return cells, fillcount, p1_score, p2_score, players, turn, player, next_turn, gameover
...
while running:
    win.fill(BLACK)  # Preenche a tela com a cor preta
    if playerinit == 1 or player == 'X':
        for event in pygame.event.get():  # Para cada evento
            reset = False  # Reseta a flag de reset
            quit = False  # Reseta a flag de saída
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:  # Se o botão do mouse foi pressionado
                pos = sendpos = event.pos  # Posição do clique
        ...
        if ccell:
            ...
            if next_turn and not res:  # Se é o próximo turno e nenhuma célula foi preenchida
                ...
                s.sendto(str.encode(f'2:{quit}:{reset}:{sendpos}:{up}:{right}:{bottom}:{left}:{player}'), (HOST, PORT))
